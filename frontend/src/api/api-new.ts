...
import { AudioMetadata, AudioChunkResponse } from "../types/audio";
...
export const audioApi = {
  ...
  ): Promise<ApiResponse<AudioChunkResponse>> => {
    try {
      if (USE_MOCK_API) {
        // Return mock data that matches the AudioChunkResponse interface
        return { 
          data: {
            success: true,
            filename: "mock-audio-file.webm",
            chunkIndex: metadata.chunkNumber,
      ...
      // Adjust to match AudioChunkResponse structure from backend
